    ...
    axios.get(`/data/boxoffice/weekly/${selectedWeek}`)
      .then((response) => {
        ...
        chartGroup.selectAll('g.area')
                    ...
                    .enter().append('g')
                      .attr('class', 'area')
                      .attr('opacity', d => { console.log(d); return 0.3; })
                      .on('mouseover', function(d, i) {
                        d3.select(this).attr('opacity', 1);
                        d3.select('#legend').selectAll(`g:nth-child(${i+1})`).attr('opacity', 1);
                        tooltip.style('left', `${33}vw`).style('top', `${yScale(d[0]) + 30}px`).style('opacity', 1).html(`<h4>${d.key}</h4>
        ...
        let legendEntry = legend.selectAll('g.legend-entry')
                                  ...
                                  .enter().append('g')
                                    ...
                                    .on('mouseover', function(d, i) {
                                      ...
                                      tooltip.style('left', `${33}vw`).style('top', `${yScale(d[2][1]) + 30}px`).style('opacity', 1).html(`<h4>${d.key}</h4>
                                        <h5>Weekly Total:</h5><p>$${data.movies[d.index].week_gross.toLocaleString()}</p>`);
                                    })
                                    .on('mouseout', function(d, i) {
                                      d3.select(this).attr('opacity', 0.5);
